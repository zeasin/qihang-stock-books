    ...
    <el-table v-loading="loading" :data="locationList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="类型" align="center" prop="warehouseType" >
        <template slot-scope="scope">
          <el-tag size="small" v-if="scope.row.type === 1">本地仓</el-tag>
          <el-tag size="small" v-else-if="scope.row.type === 2">云仓</el-tag>
      ...
      <el-table-column label="仓库类型" align="center" prop="warehouseType" >
        <template slot-scope="scope">
          ...
          <el-tag size="small" v-else-if="scope.row.warehouseType === 'JKYYC' ">吉客云云仓</el-tag>
          <el-tag size="small" v-else>未知仓库</el-tag>
        </template>
      </el-table-column>
      ...
      <el-table-column label="仓库详情" align="center" prop="remark" >
        <template slot-scope="scope">
          ...
          <el-button v-if="scope.row.warehouseType==='JKYYC'"
                     ...
                     type="text"
                     icon="el-icon-info"
                     @click="handleJkyyc(scope.row)"
          >云仓信息</el-button>
        </template>
      </el-table-column>
      <el-table-column label="分配的供应商" align="left" width="211">
        <template slot-scope="scope">
          {{ scope.row.merchantIds ? merchantList.filter(item =>
          scope.row.merchantIds.replace(/^,|,$/g, '').split(',').map(Number).includes(item.id)
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          ...
          <el-button
            ...
            icon="el-icon-share"
            @click="handleShareMerchant(scope.row)"
            v-hasPermi="['wms:location:edit']"
          >分配给商户</el-button>
          <el-row>
            <el-button style="padding-left: 6px;padding-right: 6px"
              v-if="scope.row.warehouseType==='CLOUD'"
              size="mini" plain
              type="success"
      ...
      <el-form ref="form" :model="form" :rules="rules" label-width="120px">
        ...
        <el-form-item label="仓库类型" prop="warehouseType">
          <el-select v-model="form.warehouseType" placeholder="请选择仓库类型" clearable>
            ...
            <el-option label="系统云仓" value="CLOUD"></el-option>
            <el-option label="京东云仓" value="JDYC"></el-option>
            <el-option label="吉客云云仓" value="JKYYC"></el-option>
<!--            <el-option label="其他" value="Other"></el-option>-->
          </el-select>
        </el-form-item>
        <el-form-item label="货主编码" prop="ownerNo" v-if="form.warehouseType==='JKYYC'">
          <el-input v-model="form.ownerNo" placeholder="请输入货主编码" />
        </el-form-item>
        <el-form-item label="AppKey" prop="appKey" v-if="form.warehouseType==='JDYC'">
          <el-input v-model="form.appKey" style="width: 300px"></el-input>
        </el-form-item>
        <el-form-item label="AppSecret" prop="appSecret" v-if="form.warehouseType==='JDYC'">
          <el-input v-model="form.appSecret" style="width: 300px"></el-input>
        </el-form-item>
        <el-form-item label="AccountToken" prop="accountToken" v-if="form.warehouseType==='JDYC'">
          <el-input v-model="form.accountToken" style="width: 300px"></el-input>
        </el-form-item>
        <el-form-item label="RefreshToken" prop="accountToken" v-if="form.warehouseType==='JDYC'">
          <el-input v-model="form.refreshToken" style="width: 300px"></el-input>
        </el-form-item>
        <el-form-item label="Pin" prop="bizPin" v-if="form.warehouseType==='JDYC'">
          <el-input v-model="form.bizPin" style="width: 300px"></el-input>
        </el-form-item>
        <el-form-item label="事业部编码" prop="bizId" v-if="form.warehouseType==='JDYC'">
          <el-input v-model="form.bizId" style="width: 300px"></el-input>
        </el-form-item>
        <el-form-item label="来源编号" prop="sourceNo" v-if="form.warehouseType==='JDYC'">
          <el-input v-model="form.sourceNo" style="width: 300px" placeholder="来源编号，ISV开头"></el-input>
        </el-form-item>
...
<!--        </el-form-item>-->
    ...
    <el-dialog title="分配给商户" :visible.sync="shareOpen" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="120px">
        <el-form-item label="商户" prop="merchantIds">
          <el-select v-model="form.merchantIds" placeholder="请选择商户" clearable multiple >
            <el-option
      ...
      <div slot="footer" class="dialog-footer">
        <el-button type="primary" @click="shareMerchantSubmit">确 定</el-button>
        <el-button @click="cancel">取 消</el-button>
      </div>
    </el-dialog>

    <el-dialog title="设置登陆账户" :visible.sync="openPwd" width="800px" append-to-body>
...
<!--    <el-dialog title="设置登陆账户" :visible.sync="openPwd1" width="500px" append-to-body>-->
<!--      <el-form ref="pwdform" :model="pwdform" :rules="pwdrules" label-width="88px">-->
<!--        <el-form-item label="登录名" prop="loginName">-->
<!--          <el-input v-model="pwdform.loginName" placeholder="请输入登录名" />-->
<!--        </el-form-item>-->
<!--        <el-form-item label="登录密码" prop="loginPwd">-->
<!--          <el-input v-model="pwdform.loginPwd" placeholder="请输入登录密码" />-->
<!--        </el-form-item>-->
<!--      </el-form>-->
<!--      <div slot="footer" class="dialog-footer">-->
<!--        <el-button type="primary" @click="submitPwdForm">确 定</el-button>-->
...
import {
  ...
  addLocation,
  updateLocation,
  shareMerchant, setWarehouseLoginName,getLoginAccount
...
export default {
  ...
  methods: {
    ...
    handleJkyyc(row){
      this.$router.push({path:"/wms/jky_info",query:{warehouseId:row.id}})
    },
    handleShareMerchant(row) {
      console.log("====分配商户",row)
      this.form.id = row.id
      // listMerchant({}).then(response => {
      //   this.merchantList = response.rows;
      // })
      if (row.merchantIds) {
        let numArray = row.merchantIds.replace(/^,|,$/g, '').split(',').map(Number);
        console.log("====已分配的商户",numArray)
        this.form.merchantIds = numArray
      } else this.form.merchantIds = null
      this.shareOpen = true
    },
    shareMerchantSubmit(){

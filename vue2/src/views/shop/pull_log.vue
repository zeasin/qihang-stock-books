    ...
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="100px">
      ...
      <el-form-item label="类型" prop="pullType">
        <el-select v-model="queryParams.pullType" placeholder="请选择类型" clearable @change="handleQuery">
          ...
          <el-option label="增量更新订单" value="ORDER_UPDATE"></el-option>
...
<!--      <el-table-column label="ID" align="center" prop="id" />-->
      ...
      <el-table-column label="平台" align="center" prop="shopType" >
        <template slot-scope="scope">
          ...
          <el-tag size="small" v-else-if="scope.row.shopType === 200">京东POP</el-tag>
          <el-tag size="small" v-else-if="scope.row.shopType === 280">京东自营</el-tag>
          <el-tag size="small" v-else-if="scope.row.shopType === 300">拼多多</el-tag>
          <el-tag size="small" v-else-if="scope.row.shopType === 400">抖店</el-tag>
          <el-tag size="small" v-else-if="scope.row.shopType === 500">微信小店</el-tag>
          <el-tag size="small" v-else-if="scope.row.shopType === 600">快手小店</el-tag>
          <el-tag size="small" v-else-if="scope.row.shopType === 700">小红书</el-tag>
          <el-tag size="small" v-else>{{scope.row.shopType}}</el-tag>
        </template>
      </el-table-column>
      <el-table-column label="类型" align="center" prop="pullType" >
        <template slot-scope="scope">
          <el-tag size="small" v-if="scope.row.pullType === 'GOODS'">拉取商品</el-tag>
          <el-tag size="small" v-else-if="scope.row.pullType === 'ORDER'">拉取订单</el-tag>
          <el-tag size="small" v-else-if="scope.row.pullType === 'ORDER_UPDATE'">更新订单</el-tag>
          <el-tag size="small" v-else-if="scope.row.pullType === 'REFUND'">拉取退款</el-tag>
          <el-tag size="small" v-else>{{scope.row.pullType}}</el-tag>
        </template>
      </el-table-column>

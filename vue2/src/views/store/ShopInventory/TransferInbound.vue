<template>
  <div class="transfer-inbound">
    ...
    <el-table :data="transferList" v-loading="loading" border stripe>
      ...
      <el-table-column prop="inboundTime" label="入库时间" width="160"></el-table-column>
      <el-table-column label="操作" width="120" fixed="right">
        <template slot-scope="scope">
          <el-button v-if="scope.row.status === 4" type="success" size="small" @click="confirmInbound(scope.row)">确认入库</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog title="确认入库" :visible.sync="inboundDialogVisible" width="400px">
      <div style="padding: 20px 0;">
        <p>确认对调拨单 <strong>{{ currentTransfer.applyNo }}</strong> 进行入库操作？</p>
...
export default {
  ...
  methods: {
    loadTransferList() {
      ...
      transferApi.list(params).then(res => {
        if (res.code === 200 && res.rows) {
          this.transferList = res.rows.filter(item => item.status === 4)
    ...
    submitInbound() {
      ...
      transferApi.inbound({ id: this.currentTransfer.id, operator: 'admin' }).then(res => {
        if (res.code === 200) {

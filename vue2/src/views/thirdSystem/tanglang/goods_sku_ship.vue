...
export default {
  ...
  created() {
    listMerchant({ }).then(resp => {
      ...
      listShop({ merchantId: this.queryParams.merchantId,type:911 }).then(response => {
  ...
  methods: {
    merchantChange(nv){
      ...
      listShop({merchantId:nv,type:911 }).then(response => {
        this.shopList = response.rows;
        if (this.shopList.length > 0) {
          this.queryParams.shopId = this.shopList[0].id
        }
    ...
    searchShop(query){
      if (query !== '') {
        ...
        listShop({merchantId:this.form.merchantId,type:911,name:query}).then(response => {
          this.shopList = response.rows;
          this.shopLoading = false
        })
      }
    ...
    handleAdd(){
      ...
      this.form.merchantId=this.queryParams.merchantId
      this.open = true
    },
    addGoodsDialog() {
